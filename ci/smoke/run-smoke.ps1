...
$outDir = Join-Path $rootDir "ci\smoke\out"
...
$jmxFile = Join-Path $rootDir "ci\smoke\smoke.jmx"
...
$pattern = "(?i)(^\d{4}-\d{2}-\d{2}.*\sERROR\s|stackoverflowerror|incompatibleclasschangeerror|groovyruntimeexception|conflicting module versions|^\s*Caused by:\s+.*Exception|^\s*(java|javax|org)\.[\w.$]+Exception)"
$matches = Select-String -Path $logFile -Pattern $pattern -AllMatches

if ($matches) {
    Write-Host "Smoke test failed: error patterns detected in $logFile"
    $matches | Select-Object -First 50 | ForEach-Object { Write-Host $_.Line }
    throw "JMeter smoke check failed."
}

Write-Host "Smoke test passed for JMeter $jmeterVersion."
