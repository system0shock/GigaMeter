...
| `@plan analyze` | Анализирует текущий test plan в GUI и строит текстовый разбор. | Показывает thread groups, интенсивность, состав элементов и пошаговый сценарий. |
...
### `@plan analyze`
Анализирует текущий test plan в JMeter GUI и формирует пошаговое описание сценария.

Что показывает:
- Thread Groups и их количество
- параметры нагрузки (`users`, `ramp-up`, `scheduler`, `duration`)
- оценку интенсивности (конкурентность и `users/s`)
- количество HTTP/JSR223 sampler-элементов, assertions, extractors, timers
- пошаговый сценарий по найденным sampler-элементам

Ограничения:
- Работает только в GUI-режиме JMeter (`GuiPackage` должен быть доступен)
- Если Thread Group не найден, возвращает диагностическое сообщение
